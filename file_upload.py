...
parser.add_argument("--mysql_details", help='Load settings from file in json format. Command line options override values in file.')

args = parser.parse_args()

#process which needs to be done on the filename

def process_file(filename,table,mysql_d):

    df2 = pd.read_json(mysql_d, orient ='index')
    ...
    try:
        conn = msql.connect(host=df2.host, user=df2.username, password=df2.password)#give ur username, password
...
def process_directory(directory_name,table_name,mysql_details):
    #loop through the directory
    
    for filename in os.listdir(directory_name):
        process_file(os.path.join(directory_name, filename),table_name, mysql_details)

process_directory(args.inputdir,args.destination_table,args.mysql_details)